...
module.exports = (app, io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("fight", async function (data, func) {
      if (
        SERVER_STATE.areanas[data - 1] &&
        (SERVER_STATE.areanas[data - 1].nftTokenIdOne === 0 ||
          SERVER_STATE.areanas[data - 1].nftTokenIdTwo === 0)
      ) {
        return;
      }
      const tokenOne = await nftData.findOne({
        nftId: SERVER_STATE.areanas[data - 1].nftTokenIdOne,
      });
      const tokenTwo = await nftData.findOne({
        nftId: SERVER_STATE.areanas[data - 1].nftTokenIdTwo,
      });
      ...
      let percentOneWillWin =
        (totalValueOne * 100) / (totalValueOne + totalValueTwo);
    ...
    socket.on("sendNftToAreana", async function (data, func) {
      ...
      if (SERVER_STATE.areanas[data.areanaId - 1].nftTokenIdOne === 0) {
        ...
        SERVER_STATE.areanas[data.areanaId - 1].nftTokenIdTwo === 0 &&
        SERVER_STATE.areanas[data.areanaId - 1].nftTokenIdOne !== data.tokenId
      ) {
